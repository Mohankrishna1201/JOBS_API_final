...
router.patch('/:id' ,(req,res) =>{
    ...
    const foundJob = jobs.find((newJob) => newJob.id ===id); // which job is being updated
    ...
    if(org_id) newJob.org_id = org_id;
    if(description) newJob.description = description;
    if(posted_date) newJob.posted_date = posted_date;
    if(end_date) newJob.end_date = end_date;
    if( job_type) newJob.job_type = job_type;
    if( stipend) newJob.stipend = stipend;
    if(questions) newJob.questions = questions;
    if(No_of_openings) newJob.No_of_openings = No_of_openings;

    res.send(`job with ${id} is updated and its title is ${foundJob.title} and org_id is ${foundJob.org_id}`)
